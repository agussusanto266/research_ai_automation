...
const featureName =
  featureFlagIndex !== -1 ? args[featureFlagIndex + 1] : path.basename(prdPath, ".txt");
...
function scoreSMART(text: string): FrameworkScore {
  const checks = [
    ...
    {
      label: "Measurable",
      pattern: /within \d+|≤|<=|at least \d+|exactly \d+|%|seconds|milliseconds|characters/i,
    },
    {
      label: "Achievable",
    ...
    {
      label: "Time-bound",
      pattern: /this release|sprint|version \d|date|deadline|within \d+ seconds/i,
    },
  ];
...
function scoreINVEST(text: string): FrameworkScore {
  const checks = [
    ...
    {
      label: "Testable",
      pattern: /acceptance criteria|given|when|then|verify|assert|visible|redirect/i,
    },
...
function scoreMoSCoW(text: string): FrameworkScore {
  ...
  const score = [hasMust, mustHaveWithCriteria, hasShouldOrCould, hasWontHave].filter(
    Boolean
  ).length;
...
function scoreRBT(text: string): FrameworkScore {
  ...
  const hasHighHighUnmitigated = /high\s*\|\s*high(?!.*mitigation)/im.test(text);
  const hasMitigation = /mitigation|mitigat/i.test(text);

  const passed = hasRiskSection && !hasHighHighUnmitigated;
  const score = [hasRiskSection, hasMitigation, !hasHighHighUnmitigated].filter(Boolean).length;
...
function scoreBVAEP(text: string): FrameworkScore {
  ...
  const hasBoundaryValues = /\blength \d+|bva value|\d+ character|\d+ char/i.test(text);
  const hasEPClasses = /valid ep|invalid ep|equivalence class/i.test(text);

  const score = [hasInputSection, hasMinMax, hasBoundaryValues, hasEPClasses].filter(
    Boolean
  ).length;
...
if (!frameworks[0].passed)
  failedItems.push("SMART: fewer than 4/5 criteria met — add measurable targets or time bounds");
if (!frameworks[1].passed)
  failedItems.push("INVEST: fewer than 4/7 criteria met — split stories or add estimability");
if (!frameworks[2].passed)
  failedItems.push("MoSCoW: Must Have items lack clear acceptance criteria");
if (!frameworks[3].passed)
  failedItems.push("RBT: unmitigated High×High risk detected or no risk table present");
if (!frameworks[4].passed)
  failedItems.push("BVA/EP: input fields missing boundary value or equivalence class definitions");
...
if (verdict === "NEEDS REVISION") {
  ...
  const feedbackFile = path.join(feedbackDir, `${featureName}_prd_${today}.txt`);
