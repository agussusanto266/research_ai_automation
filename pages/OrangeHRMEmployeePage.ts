...
const EMPLOYEE_ID_INPUT_CANDIDATES: LocatorCandidate[] = [
  ...
  { name: "secondary-xpath",kind: "xpath", value: "//label[text()='Employee Id']/following::input[1]" },
...
const ADD_BUTTON_CANDIDATES: LocatorCandidate[] = [
  ...
  { name: "fallback-xpath", kind: "xpath",value: "//button[contains(@class,'oxd-button')][normalize-space()='Add']" }
];
...
export class OrangeHRMEmployeePage extends BasePage {
  ...
  async goto(baseUrl: string): Promise<void> {
    await this.page.goto(`${baseUrl}/web/index.php/pim/viewEmployeeList`, { waitUntil: "domcontentloaded", timeout: 60000 });
  ...
  private async waitForTableOrNoRecords(timeout = 10000): Promise<void> {
    await Promise.race([
      this.page.locator(TABLE_BODY_ROW_CSS).first().waitFor({ state: "visible", timeout }).then(() => true).catch(() => false),
      this.noRecordsLocator().waitFor({ state: "visible", timeout }).then(() => true).catch(() => false),
    ]);
  }
  ...
  private async waitForLoadingToFinish(): Promise<void> {
    ...
    await this.page.locator(".oxd-loading-spinner, .oxd-form-loader").first()
