...
Given(
  ...
  async function (this: CustomWorld, username: string) {
    ...
    assert.ok(
      password !== undefined,
      `No password configured for user "${username}" in test-data/saucedemo-users.ts`
    );
    ...
    if (sessionState) {
      ...
      if (/inventory\.html/.test(this.page.url())) {
        this.scenarioLogs.push(
          `[session-cache] ${username} authenticated via cached storage state`
        );
        return;
      }
      ...
      this.scenarioLogs.push(
        `[session-cache] ${username} cached state invalid — falling back to UI login`
      );
    }
